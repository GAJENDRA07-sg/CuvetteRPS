...
const meta = {
  ...
  paper: { color: "var(--paper)", icon: "paper" },
...
function renderIcon(id, choice) {
  ...
  el.innerHTML = `<div class="icon-inner"><img src="./src/${meta[choice].icon}.png"></div>`;
}

function setResultUI(msg, showSub, winner) {
  document.getElementById("result-text").innerText = msg;
  document.getElementById("against-text").style.display = showSub
    ? "block"
    : "none";
  document
    .getElementById("user-ripple")
    .classList.toggle("winner-ripple", winner === "user");
  document
    .getElementById("pc-ripple")
    .classList.toggle("winner-ripple", winner === "pc");
}
...
document.getElementById("rules-open").onclick = () =>
